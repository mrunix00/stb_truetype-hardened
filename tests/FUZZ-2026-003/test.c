...
void test_valid_signature_but_truncated_directory_is_rejected_without_oob_read(void)
{
    stbtt_fontinfo info;
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, ok, "truncated SFNT table directory must be rejected safely");
...
int main(void)
{
    load_payload_or_fail("truncated_valid_sfnt_tabledir.ttf");

    UNITY_BEGIN();
    RUN_TEST(test_valid_signature_but_truncated_directory_is_rejected_without_oob_read);
