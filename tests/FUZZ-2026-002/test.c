...
void test_consistent_header_but_truncated_tabledir_is_rejected_without_oob_read(void)
{
    stbtt_fontinfo info;
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, ok, "truncated but structurally-consistent table directory must be rejected");
...
int main(void)
{
    load_payload_or_fail("truncated_consistent_tabledir.ttf");

    UNITY_BEGIN();
    RUN_TEST(test_consistent_header_but_truncated_tabledir_is_rejected_without_oob_read);
